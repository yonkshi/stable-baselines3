...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0 and 'l' in self.ep_info_buffer[0].keys():
            logger.record("rollout/ep_rew_mean", safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer]))
            logger.record("rollout/ep_len_mean", safe_mean([ep_info["l"] for ep_info in self.ep_info_buffer if 'l' in ep_info.keys()]))
